...
interface PilotClient {
    @GET("{id}")
    @Wrapped(path = ["data"])
    fun getPilot(@Path("id") pilotId: String = AirMap.userId): Response<Pilot>

    @PATCH("{id}")
    @Wrapped(path = ["data"])
    fun updatePilot(
        @Body updatedPilot: Pilot,
        @Path("id") pilotId: String = AirMap.userId
    ): Response<Pilot>

    @POST("{id}/phone/send_token")
    fun sendVerificationToken(@Path("id") pilotId: String = AirMap.userId): Response<Unit>
    ...
    @GET("{id}/aircraft")
    @Wrapped(path = ["data"])
    fun getAllAircraft(@Path("id") pilotId: String = AirMap.userId): Response<List<Aircraft>>
